...
class PlotTab:
    ...
    def __init__(
        ...
        self.add_layer_btn = pn.widgets.Button(label="+ Add layer", width=120)
        ...
        self.dup_btn = pn.widgets.Button(label="⧉ Duplicate", width=120)
    ...
    def _rebuild_layers(self) -> None:
        ...
        for i, row in enumerate(self.rows):
            up = pn.widgets.Button(label="↑", width=32)
            up.on_click(lambda _e, r=row: self.move_layer(r, -1))
            down = pn.widgets.Button(label="↓", width=32)
            down.on_click(lambda _e, r=row: self.move_layer(r, 1))
            rm = pn.widgets.Button(label="✕", color="danger", width=32)
            rm.on_click(lambda _e, r=row: self.remove_layer(r))
            blocks.append(
                pn.Column(
                    pn.pane.Markdown(f"**Layer {i + 1}**", margin=(4, 0, -10, 0)),
                    row.layout(pn.Row(up, down, rm)),
    ...
    def _update(self, _event: object = None) -> None:
        ...
        try:
            ...
            spec = self.to_spec()
            try:
                self.plot_area[:] = [pn.panel(self.view())]
